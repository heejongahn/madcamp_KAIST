...
public class TagView extends RelativeLayout {
    ...
    /** tag list */
    private List<TagItem> mTagItems = new ArrayList<TagItem>();
    ...
    private void drawTags() {
        ...
        int index_bottom=1;// The TagItem to add below
        int index_header=1;// The header tag of this line
        TagItem tag_Item_pre =null;
        for (TagItem item : mTagItems) {
            ...
            tagLayout.setBackgroundDrawable(getSelector(tagItem));

            // tagItem text
            TextView tagView = (TextView) tagLayout.findViewById(R.id.tv_tag_item_contain);
            tagView.setText(tagItem.text);
            ...
            tagView.setTextColor(tagItem.tagTextColor);
            tagView.setTextSize(TypedValue.COMPLEX_UNIT_SP , tagItem.tagTextSize);
            tagLayout.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (mClickListener != null) {
                        mClickListener.onTagClick(tagItem, position);
                    }
                }
            });

            // calculate　of tagItem layout width
            float tagWidth = tagView.getPaint().measureText(tagItem.text) + textPaddingLeft + textPaddingRight;
            ...
            if (tagItem.isDeletable) {
                ...
                deletableView.setText(tagItem.deleteIcon);
                ...
                deletableView.setTextColor(tagItem.deleteIndicatorColor);
                deletableView.setTextSize(TypedValue.COMPLEX_UNIT_SP, tagItem.deleteIndicatorSize);
                deletableView.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        //TagView.this.remove(position);
                        if (mDeleteListener != null) {
                            TagItem targetTagItem = tagItem;
                            mDeleteListener.onTagDeleted(TagView.this, targetTagItem, position);
                        }
                    }
                });
                tagWidth += deletableView.getPaint().measureText(tagItem.deleteIcon) +textPaddingLeft + textPaddingRight;
            ...
            if (mWidth <= total + tagWidth + Utils.dipToPx(this.getContext(),Constants.LAYOUT_WIDTH_OFFSET)) {
                ...
                if (listIndex!=index_header) {
                    ...
                    total += tagMargin;
                    if (tag_Item_pre !=null&& tag_Item_pre.tagTextSize< tagItem.tagTextSize) {
                        index_bottom = listIndex;
                    }
                }
    ...
    private Drawable getSelector(TagItem tagItem) {
        if (tagItem.background!=null)return tagItem.background;
        StateListDrawable states = new StateListDrawable();
        GradientDrawable gd_normal = new GradientDrawable();
        gd_normal.setColor(tagItem.layoutColor);
        gd_normal.setCornerRadius(tagItem.radius);
        if (tagItem.layoutBorderSize>0){
            gd_normal.setStroke(Utils.dipToPx(getContext(), tagItem.layoutBorderSize), tagItem.layoutBorderColor);
        }
        GradientDrawable gd_press = new GradientDrawable();
        gd_press.setColor(tagItem.layoutColorPress);
        gd_press.setCornerRadius(tagItem.radius);
    ...
    public void addTags(ArrayList<TagItem> tagItems){
    ...
    public void addTags(String[] tags){
        ...
        for(String item:tags){
            TagItem tagItem = new TagItem(item);
            addTag(tagItem);
        }
    }
     ...
     * get tag list
     *
     * @return mTagItems TagObject List
     */
    ...
    public void remove(int position) {
        if (position < mTagItems.size()) {
            mTagItems.remove(position);
            drawTags();
        }
    }
